...
def plotstft(audiopath, model, binsize=2 ** 10, name='tmp.png', alpha=1, quantity=1, img_size=None):
    """plot spectrogram"""
...
model = tf.keras.models.load_model(r"D:\speechrecogn\model9.78-0.264-0.905.hdf5")

for audio in audios:    
    img = plotstft(audio, model)
    arr = np.asarray(img)
    arr = np.reshape(arr, (1, arr.shape[1], arr.shape[0], 1))
...
model2 = tf.keras.models.load_model(r"D:\speechrecogn\model7.117-0.313-0.899.hdf5")
...
res2 = model2.predict(x_va[0:1])

print(res2)
print(np.array(y_va[0:1]))